...
if (!checkRelativePath($_POST['path']) || strpos($_POST['path'], '/') === 0) {
    responseRFM(rfm_trans('wrong path') . AddErrorLocation(), 400)->send();
    exit;
}
...
if (!check_extension($info['extension'], $config)) {
    responseRFM(rfm_trans('wrong extension') . AddErrorLocation(), 400)->send();
    exit;
}
...
if ($ftp) {
    ...
    if (!file_exists($path . $name)) {
        responseRFM(rfm_trans('File_Not_Found') . AddErrorLocation(), 404)->send();
        exit;
    }
