...
<body>
    ...
    <input type="hidden" id="insert_folder_name" value="<?php echo rfm_trans('Insert_Folder_Name');?>" />
    <input type="hidden" id="rename_existing_folder" value="<?php echo rfm_trans('Rename_existing_folder');?>" />
    <input type="hidden" id="new_folder" value="<?php echo rfm_trans('New_Folder');?>" />
    <input type="hidden" id="ok" value="<?php echo rfm_trans('OK');?>" />
    <input type="hidden" id="cancel" value="<?php echo rfm_trans('Cancel');?>" />
    <input type="hidden" id="rename" value="<?php echo rfm_trans('Rename');?>" />
    <input type="hidden" id="lang_duplicate" value="<?php echo rfm_trans('Duplicate');?>" />
    ...
    <input type="hidden" id="lang_show_url" value="<?php echo rfm_trans('Show_url');?>" />
    ...
    <input type="hidden" id="lang_copy" value="<?php echo rfm_trans('Copy');?>" />
    <input type="hidden" id="lang_cut" value="<?php echo rfm_trans('Cut');?>" />
    <input type="hidden" id="lang_paste" value="<?php echo rfm_trans('Paste');?>" />
    <input type="hidden" id="lang_paste_here" value="<?php echo rfm_trans('Paste_Here');?>" />
    <input type="hidden" id="lang_paste_confirm" value="<?php echo rfm_trans('Paste_Confirm');?>" />
    <input type="hidden" id="lang_files" value="<?php echo rfm_trans('Files');?>" />
    <input type="hidden" id="lang_folders" value="<?php echo rfm_trans('Folders');?>" />
    <input type="hidden" id="lang_files_on_clipboard" value="<?php echo rfm_trans('Files_ON_Clipboard');?>" />
    <input type="hidden" id="clipboard" value="<?php echo ((isset($_SESSION['RF']['clipboard']['path']) && trim($_SESSION['RF']['clipboard']['path']) != null) ? 1 : 0);?>" />
    <input type="hidden" id="lang_clear_clipboard_confirm" value="<?php echo rfm_trans('Clear_Clipboard_Confirm');?>" />
    <input type="hidden" id="lang_file_permission" value="<?php echo rfm_trans('File_Permission');?>" />
    ...
    <input type="hidden" id="lang_lang_change" value="<?php echo rfm_trans('Lang_Change');?>" />
    <input type="hidden" id="edit_text_files_allowed" value="<?php if($config['edit_text_files']) echo 1; else echo 0;?>" />
    <input type="hidden" id="lang_edit_file" value="<?php echo rfm_trans('Edit_File');?>" />
    <input type="hidden" id="lang_new_file" value="<?php echo rfm_trans('New_File');?>" />
    <input type="hidden" id="lang_filename" value="<?php echo rfm_trans('Filename');?>" />
    <input type="hidden" id="lang_file_info" value="<?php echo fix_strtoupper(rfm_trans('File_info'));?>" />
    <input type="hidden" id="lang_edit_image" value="<?php echo rfm_trans('Edit_image');?>" />
    <input type="hidden" id="lang_error_upload" value="<?php echo rfm_trans('Error_Upload');?>" />
    <input type="hidden" id="lang_select" value="<?php echo rfm_trans('Select');?>" />
    <input type="hidden" id="lang_extract" value="<?php echo rfm_trans('Extract');?>" />
...
<div class="uploader">
    <div class="flex">
        <div class="text-center">
            <button class="btn btn-inverse close-uploader"><i class="icon-backward icon-white"></i> <?php echo rfm_trans('Return_Files_List')?></button>
        </div>
        <div class="space10"></div>
        <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
            <div class="container1">
            <ul class="nav nav-tabs">
                <li class="active"><a href="#baseUpload" data-toggle="tab"><?php echo rfm_trans('Upload_base');?></a></li>
                <?php if($config['url_upload']){ ?>
                <li><a href="#urlUpload" data-toggle="tab"><?php echo rfm_trans('Upload_url');?></a></li>
                <?php } ?>
            </ul>
                ...
                <div class="tab-pane active" id="baseUpload">
                    ...
                    <form id="fileupload" action="" method="POST" enctype="multipart/form-data">
                        <div class="container2">
                            <div class="fileupload-buttonbar">
                                ...
                                <div class="text-center">
                                    ...
                                    <span class="btn btn-success fileinput-button">
                                        ...
                                        <span><?php echo rfm_trans('Upload_add_files');?></span>
                                    ...
                                    <button type="submit" class="btn btn-warning start">
                                        ...
                                        <span><?php echo rfm_trans('Upload_start');?></span>
                                    </button>
                            ...
                            <div class="upload-help"><?php echo rfm_trans('Upload_base_help');?></div>
                        </div>
                    </form>
                ...
                <div class="tab-pane" id="urlUpload">
                    ...
                    <form class="form-horizontal">
                        <div class="control-group">
                            <label class="control-label" for="url"><?php echo rfm_trans('Upload_url');?></label>
                            <div class="controls">
                                <input type="text" class="input-block-level" id="url" placeholder="<?php echo rfm_trans('Upload_url');?>">
                            </div>
                        </div>
                        <div class="control-group">
                            <div class="controls">
                                <button class="btn btn-warning" id="uploadURL"><?php echo  rfm_trans('Upload_file');?></button>
                            </div>
                        </div>
                    </form>
                </div>
...
foreach ($files as $file_array) {
    ...
    if (
        ...
        or (
            ...
            and $file_array[ 'extension' ] == fix_strtolower(rfm_trans('Type_dir'))
        )
        or (
        ...
        or (
            ...
            and $file_array['extension'] != fix_strtolower(rfm_trans('Type_dir'))
            and (
                !check_file_extension($file_array['extension'], $config)
                or (
                    $filter != ''
                    and $n_files > $config['file_number_limit_js']
                    and stripos($file, $filter) === false
        ...
        <div class="filters">
          <div class="row-fluid">
            <div class="span4 half">
                <?php if($config['upload_files']){ ?>
                <button class="tip btn upload-btn btn-success btn-large" title="<?php echo  rfm_trans('Upload_file');?>"><i class="rficon-upload"></i><?php echo  rfm_trans('Upload_file');?></button>
                <?php } ?>
                <?php if($config['create_text_files']){ ?>
                <button class="tip btn create-file-btn btn-large" title="<?php echo  rfm_trans('New_File');?>"><i class="icon-plus"></i><i class="icon-file"></i></button>
                <?php } ?>
                <?php if($config['create_folders']){ ?>
                <button class="tip btn new-folder btn-large" title="<?php echo  rfm_trans('New_Folder')?>"><i class="icon-plus"></i><i class="icon-folder-open"></i></button>
                <?php } ?>
                <?php if($config['copy_cut_files'] || $config['copy_cut_dirs']){ ?>
                <button class="tip btn paste-here-btn btn-large" title="<?php echo rfm_trans('Paste_Here');?>"><i class="rficon-clipboard-apply"></i></button>
                <button class="tip btn clear-clipboard-btn btn-large" title="<?php echo rfm_trans('Clear_Clipboard');?>"><i class="rficon-clipboard-clear"></i></button>
                <?php } ?>
                <div id="multiple-selection" style="display:none;">
                <?php if($config['multiple_selection']){ ?>
                <?php if($config['delete_files']){ ?>
                <button class="tip btn multiple-delete-btn btn-large" title="<?php echo rfm_trans('Erase');?>" data-confirm="<?php echo rfm_trans('Confirm_del');?>"><i class="icon-trash"></i></button>
                <?php } ?>
                <button class="tip btn multiple-select-btn btn-large" title="<?php echo rfm_trans('Select_All');?>"><i class="icon-check"></i></button>
                <button class="tip btn multiple-deselect-btn btn-large" title="<?php echo rfm_trans('Deselect_All');?>"><i class="icon-ban-circle"></i></button>
                <?php if($apply_type!="apply_none" && $config['multiple_selection_action_button']){ ?>
                <button class="btn multiple-action-btn btn-inverse btn-large" data-function="<?php echo $apply_type;?>"><?php echo rfm_trans('Select'); ?></button>
                <?php } ?>
                <?php } ?>
                </div>
            </div>
            <div class="span2 half view-controller">
                <button class="btn tip<?php if($view==0) echo " btn-inverse btn-large";?>" id="view0" data-value="0" title="<?php echo rfm_trans('View_boxes');?>"><i class="icon-th <?php if($view==0) echo "icon-white";?>"></i></button>
                <button class="btn tip<?php if($view==1) echo " btn-inverse btn-large";?>" id="view1" data-value="1" title="<?php echo rfm_trans('View_list');?>"><i class="icon-align-justify <?php if($view==1) echo "icon-white btn-large";?>"></i></button>
                <button class="btn tip<?php if($view==2) echo " btn-inverse btn-large";?>" id="view2" data-value="2" title="<?php echo rfm_trans('View_columns_list');?>"><i class="icon-fire <?php if($view==2) echo "icon-white";?>"></i></button>
            </div>
        <div class="nav-collapse collapse">

            <div class="span6 entire types">
                <span><?php echo rfm_trans('Filters');?>:</span>
                <?php if($_GET['type']!=1 && $_GET['type']!=3 && $config['show_filter_buttons']){ ?>
                    <?php if(count($config['ext_file'])>0 or false){ ?>
                <input id="select-type-1" name="radio-sort" type="radio" data-item="ff-item-type-1" checked="checked"  class="hide"  />
                <label id="ff-item-type-1" title="<?php echo rfm_trans('Files');?>" for="select-type-1" class="tip btn ff-label-type-1"><i class="icon-file"></i></label>
                    <?php } ?>
                    <?php if(count($config['ext_img'])>0 or false){ ?>
                <input id="select-type-2" name="radio-sort" type="radio" data-item="ff-item-type-2" class="hide"  />
                <label id="ff-item-type-2" title="<?php echo rfm_trans('Images');?>" for="select-type-2" class="tip btn ff-label-type-2"><i class="icon-picture"></i></label>
                    <?php } ?>
                    <?php if(count($config['ext_misc'])>0 or false){ ?>
                <input id="select-type-3" name="radio-sort" type="radio" data-item="ff-item-type-3" class="hide"  />
                <label id="ff-item-type-3" title="<?php echo rfm_trans('Archives');?>" for="select-type-3" class="tip btn ff-label-type-3"><i class="icon-inbox"></i></label>
                    <?php } ?>
                ...
                <label id="ff-item-type-4" title="<?php echo rfm_trans('Videos');?>" for="select-type-4" class="tip btn ff-label-type-4"><i class="icon-film"></i></label>
                    <?php } ?>
                    <?php if(count($config['ext_music'])>0 or false){ ?>
                <input id="select-type-5" name="radio-sort" type="radio" data-item="ff-item-type-5" class="hide"  />
                <label id="ff-item-type-5" title="<?php echo rfm_trans('Music');?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
                    <?php } ?>
                <?php } ?>
                <input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype');?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(rfm_trans('Text_filter'));?>..." value="<?php echo $filter;?>"/><?php if($n_files>$config['file_number_limit_js']){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

                <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
                <label id="ff-item-type-all" title="<?php echo rfm_trans('All');?>" <?php if($_GET['type']==1 || $_GET['type']==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><?php echo rfm_trans('All');?></label>
...
<!-- header div end -->
   ...
   <li class="pull-right">
      ...
      <ul class="dropdown-menu pull-left sorting">
         <li class="text-center"><strong><?php echo rfm_trans('Sorting') ?></strong></li>
      <li><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="name"><?php echo rfm_trans('Filename');?></a></li>
      <li><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="date"><?php echo rfm_trans('Date');?></a></li>
      <li><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="size"><?php echo rfm_trans('Size');?></a></li>
      <li><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="extension"><?php echo rfm_trans('Type');?></a></li>
      </ul>
      </div>
   </li>
   <li><small class="hidden-phone">(<span id="files_number"><?php echo $current_files_number."</span> ".rfm_trans('Files')." - <span id='folders_number'>".$current_folders_number."</span> ".rfm_trans('Folders');?>)</small></li>
   <?php if($config['show_total_size']){ ?>
   <li><small class="hidden-phone"><span title="<?php echo rfm_trans('total size').$config['MaxSizeTotal'];?>"><?php echo rfm_trans('total size').": ".makeSize($sizeCurrentFolder).(($config['MaxSizeTotal'] !== false && is_int($config['MaxSizeTotal']))? '/'.$config['MaxSizeTotal'].' '.rfm_trans('MB'):'');?></span></small>
   </li>
   <?php } ?>
   </ul>
   </div>
   ...
   <div class="span12">
      ...
      <?php }else{ ?>
      <h4 id="help"><?php echo rfm_trans('Swipe_help');?></h4>
      ...
      <div class="file-name"><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="name"><?php echo rfm_trans('Filename');?></a></div>
      <div class="file-date"><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="date"><?php echo rfm_trans('Date');?></a></div>
      <div class="file-size"><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="size"><?php echo rfm_trans('Size');?></a></div>
      <div class='img-dimension'><?php echo rfm_trans('Dimension');?></div>
      <div class='file-extension'><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="extension"><?php echo rfm_trans('Type');?></a></div>
      <div class='file-operations'><?php echo rfm_trans('Operations');?></div>
      </div>
      <?php } ?>
        ...
        foreach ($files as $file_array) {
            $file=$file_array['file'];
            if($file == '.' || ( substr($file, 0, 1) == '.' && isset( $file_array[ 'extension' ] ) && $file_array[ 'extension' ] == fix_strtolower(rfm_trans( 'Type_dir' ) )) || (isset($file_array['extension']) && $file_array['extension']!=fix_strtolower(rfm_trans('Type_dir'))) || ($file == '..' && $subdir == '') || in_array($file, $config['hidden_folders']) || ($filter!='' && $n_files>$config['file_number_limit_js'] && $file!=".." && stripos($file,$filter)===false)){
                continue;
            }
            ...
            <?php }else{ ?>
                    ...
                    <input type="hidden" class="extension" value="<?php echo fix_strtolower(rfm_trans('Type_dir'));?>"/>
                    <div class="file-date"><?php echo date(rfm_trans('Date_type'),$file_array['date']);?></div>
                    ...
                    <?php } ?>
                    <div class='file-extension'><?php echo fix_strtolower(rfm_trans('Type_dir'));?></div>
                    <figcaption>
                        <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($config['rename_folders'] && !$file_prevent_rename) echo "rename-folder";?>" title="<?php echo rfm_trans('Rename')?>" data-folder="1" data-permissions="<?php echo $file_array['permissions']; ?>">
                        <i class="icon-pencil <?php if(!$config['rename_folders'] || $file_prevent_rename) echo 'icon-white';?>"></i></a>
                        <a href="javascript:void('')" class="tip-left erase-button btn-danger btn-large <?php if($config['delete_folders'] && !$file_prevent_delete) echo "delete-folder";?>" title="<?php echo rfm_trans('Erase')?>" data-confirm="<?php echo rfm_trans('Confirm_Folder_del');?>" >
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                if($file == '.' || $file == '..' || $file_array['extension']==fix_strtolower(rfm_trans('Type_dir')) || !check_extension($file_array['extension'],$config) || ($filter!='' && $n_files>$config['file_number_limit_js'] && stripos($file,$filter)===false))
                    continue;
                foreach ( $config['hidden_files'] as $hidden_file ) {
                    if ( fnmatch($hidden_file, $file, FNM_PATHNAME) ) {
                        continue 2;
...
?>
            ...
            <?php if($config['multiple_selection']){ ?><div class="selector">
                ...
                <figcaption>
                    ...
                    <a title="<?php echo rfm_trans('Download')?>" class="tip-right" href="javascript:void('')" <?php if($config['download_files']) echo "onclick=\"$('#form".$nu."').submit();\"" ?>><i class="icon-download <?php if(!$config['download_files']) echo 'icon-white'; ?>"></i></a>

                    <?php if($is_img && $src_thumb!=""){ ?>
                    <a class="tip-right preview" title="<?php echo rfm_trans('Preview')?>" data-featherlight="image" href="<?php echo $src;?>"><i class=" icon-eye-open"></i></a>
                    ...
                    title="<?php echo rfm_trans('Preview')?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif(in_array($file_array['extension'],$config['cad_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo rfm_trans('Preview')?>" data-url="ajax_calls.php?action=cad_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif($config['preview_text_files'] && in_array($file_array['extension'],$config['previewable_text_file_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo rfm_trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif($config['googledoc_enabled'] && in_array($file_array['extension'],$config['googledoc_file_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo rfm_trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="docs.google.com;" ><i class=" icon-eye-open"></i></a>
                    <?php }else{ ?>
                    <a class="preview disabled"><i class="icon-eye-open icon-white"></i></a>
                    <?php } ?>
                    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($config['rename_files'] && !$file_prevent_rename) echo "rename-file";?>" title="<?php echo rfm_trans('Rename')?>" data-folder="0" data-permissions="<?php echo $file_array['permissions']; ?>">
                    <i class="icon-pencil <?php if(!$config['rename_files'] || $file_prevent_rename) echo 'icon-white';?>"></i></a>

                    <a href="javascript:void('')" class="tip-left erase-button <?php if($config['delete_files'] && !$file_prevent_delete) echo "delete-file";?>" title="<?php echo rfm_trans('Erase')?>" data-confirm="<?php echo rfm_trans('Confirm_del');?>">
...
<script>
    ...
    <div class="modal hide" id="previewAV">
        <div class="modal-header">
            ...
            <h3><?php echo rfm_trans('Preview'); ?></h3>
        </div>
    ...
    <?php if ( $config['tui_active'] ) { ?>
        ...
        if (image_editor) {
            ...
            replaceWith('<button class="tui-image-editor-exit-btn" ><?php echo rfm_trans('Image_Editor_Exit');?></button>');
            $('.tui-image-editor-exit-btn').on('click', function() {
                exitTUI();
            });
            //Replace download button with save
            $('.tui-image-editor-download-btn').
            replaceWith('<button class="tui-image-editor-save-btn" ><?php echo rfm_trans('Image_Editor_Save');?></button>');
